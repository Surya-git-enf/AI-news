...
WS      /ws/chat         Streaming version of /chat — sends short "status" progress
                         frames while Nova works, then one "final" frame.
                         Send: {email, message, conversation_name?}
POST    /update_suggestions
                         Updates user's category suggestions based on three lists.
GET     /sort            Deletes `news` rows older than 30 days.
                         Returns {deleted, currently_have}.
──────────────────────────────────────────────────────────────────────────
...
def derive_display_name(email: str) -> str:
    """Take the part before @, split on ._-, take the first token, strip trailing
    digits, title-case it. Falls back to 'User' if that leaves nothing usable."""
    try:
...
def root():
    return {
        ...
        "endpoints": {
            ...
            "WS /ws/chat": "WebSocket endpoint for streaming Nova chat",
            "POST /update_suggestions": "Update user's category suggestions",
            "GET /sort": "Delete news rows older than 30 days",
        },
    }
...
def chat(req: ChatRequest):
    ...
    display_name = derive_display_name(req.email)
    chat_list = append_messages(chat_list, conversation_name, [
        {display_name: req.message},
        {"Nova": reply_text},
    ])
...
THINKING_MESSAGES = [
    "Pondering…",
    "Mulling it over…",
    "Warming up the newsroom…",
    "Rustling papers…",
    "Getting up to speed…",
]
SEARCHING_LOCAL_MESSAGES = [
    "Archive-diving…",
    "Flipping through back issues…",
    "Checking the files…",
    "Scanning the desk…",
]
SEARCHING_RSS_MESSAGES = [
    "Patching into the wire…",
    "Going live…",
    "Tuning in…",
    "Fetching the freshest take…",
]
WRITING_MESSAGES = [
    "Drafting…",
    "Typing it up…",
        ...
        while True:
            ...
            if is_news_request:
                ...
                if context_rows:
                    ...
                    # No local results. NOTE: there is no live-RSS fallback wired up yet —
                    # this only sends the "checking the wires" status for flavor, it does not
                    # actually fetch anything. Being honest about that here: grounded_in stays
                    # "none" rather than falsely claiming "rss". If you want a real live-RSS
                    # tool for chat (separate from the /news ingestion feeds), that's a
                    # follow-up feature, not implemented in this file yet.
                    await websocket.send_json({
                        "type": "status",
                        "stage": "searching_rss",
...
def update_suggestions(req: UpdateSuggestionsRequest):
    ...
    existing = _as_list(existing_raw)
...
@app.get("/sort")
def sort_news():
    """Delete news rows older than 30 days. Returns how many were removed and how
    many remain — uses a count query rather than pulling every row into memory."""
    cutoff = (datetime.utcnow() - timedelta(days=30)).isoformat()

    old_rows = supabase.table("news").select("id").lt("published_date", cutoff).execute()
    old_ids = [r["id"] for r in (old_rows.data or [])]

    deleted = 0
    if old_ids:
        supabase.table("news").delete().in_("id", old_ids).execute()
        deleted = len(old_ids)

    remaining = supabase.table("news").select("id", count="exact").execute()
    currently_have = remaining.count if remaining.count is not None else len(remaining.data or [])

    return {"deleted": deleted, "currently_have": currently_have}
